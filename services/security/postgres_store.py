...
CONNECTOR_TYPES = ("github", "jira", "aws", "openai", "railway", "vercel", "azure")
...
class SecureDatabaseManager:
    ...
    def _ensure_database_ready(self) -> None:
        ...
        try:
            self.adapter.execute_update(
                "ALTER TABLE assignments ADD COLUMN IF NOT EXISTS portfolio_id TEXT DEFAULT 'default'"
            )
            self.adapter.execute_update(
                "UPDATE assignments SET portfolio_id = 'default' WHERE portfolio_id IS NULL"
            )
        except Exception as e:
            logger.warning("portfolio_id migration skipped: %s", e)
        ...
        # Repo intelligence tables (idempotent; gated at runtime by ENABLE_REPO_INTELLIGENCE).
        try:
            from services.security.canonical_schema import TABLES
        ...
        try:
            ...
            repo_intel_ddls = [
                ...
                f"ALTER TABLE {TABLES.REPOSITORY_SNAPSHOTS} ADD COLUMN IF NOT EXISTS repository_id TEXT REFERENCES {TABLES.REPOSITORIES}(repository_id) ON DELETE CASCADE",
                ...
                f"ALTER TABLE {TABLES.REPOSITORY_CODE_INDEX} ADD COLUMN IF NOT EXISTS repository_id TEXT REFERENCES {TABLES.REPOSITORIES}(repository_id) ON DELETE CASCADE",
                ...
                f"ALTER TABLE {TABLES.REPOSITORY_BASELINE_METRICS} ADD COLUMN IF NOT EXISTS repository_id TEXT REFERENCES {TABLES.REPOSITORIES}(repository_id) ON DELETE CASCADE",
        ...
        try:
            ...
            for ddl in repository_snapshot_ddls:
                self.adapter.execute_update(ddl)
        except Exception as e:
            logger.warning("repository_snapshots migration skipped: %s", e)

        # Repository code index table (idempotent; gated at runtime by ENABLE_REPO_INTELLIGENCE).
        try:
            from services.security.canonical_schema import TABLES
            ...
            for ddl in code_index_ddls:
                self.adapter.execute_update(ddl)
        except Exception as e:
            logger.warning("repository_code_index migration skipped: %s", e)

        # Repository baseline metrics table (idempotent; gated at runtime by ENABLE_REPO_INTELLIGENCE).
        try:
            from services.security.canonical_schema import TABLES

            baseline_metrics_ddls = [
                f"""
            ...
            for ddl in baseline_metrics_ddls:
                self.adapter.execute_update(ddl)
        except Exception as e:
            logger.warning("repository_baseline_metrics migration skipped: %s", e)
        ...
        try:
            ...
            query = """
                ...
                (assignment_id, workspace_id, name, description, team_size, monthly_burn_rate,
                 target_monthly_burn, status, portfolio_id, metrics_config, updated_at)
            ...
            params = (
                ...
                assignment_data.get("portfolio_id") or "default",
                metrics_json,
            )
            self.adapter.execute_update(query, params)
            return True
        ...
        try:
            rows = self.adapter.execute_query(
                """SELECT assignment_id, workspace_id, name, description, team_size,
                          monthly_burn_rate, target_monthly_burn, status, portfolio_id, metrics_config,
            ...
            return {
                ...
                "status": row["status"],
                "portfolio_id": row.get("portfolio_id") or "default",
