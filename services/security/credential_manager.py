...
class SecureCredentialManager:
    ...
    def _get_encryption_key(self) -> Fernet:
        ...
        if not master_key:
            ...
            logger.warning(
                "Using development credential key. Set CREDENTIAL_MASTER_KEY environment variable for production!"
            )
    ...
    def store_assignment_credentials(
        ...
        try:
            ...
            cred_file = (
                self.credentials_dir
                / f"assignment_{self._safe_filename(workspace_id)}_{self._safe_filename(assignment_id)}_{connector_type}.enc"
            )
            with open(cred_file, "wb") as f:
                f.write(encrypted_data)
    ...
    def migrate_existing_credentials(self) -> Dict[str, Any]:
        ...
        """
        migration_report = {"users_migrated": 0, "assignments_migrated": 0, "errors": []}
                        ...
                        if assignments_dir.exists():
                            for assignment_file in assignments_dir.glob("*.json"):
                                try:
                                    ...
                                    workspace_id = assignment_data.get(
                                    ...
                                    for connector_type, config in metrics_config.items():
                                        ...
                                        if credentials and any(credentials.values()):
                                            if self.store_assignment_credentials(
                                                workspace_id,
                                                assignment_id,
                                                connector_type,
                                                credentials,
                                    ...
                                    migration_report["errors"].append(
                                        f"Assignment {assignment_file.name}: {str(e)}"
                                    )
