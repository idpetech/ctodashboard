        ...
        try:
            ...
            for instance in instances.get("instances", []):
                ...
                instance_detail = {
                    ...
                    "bundle_id": bundle_id,
                    "state": state,
                    "created_at": instance.get("createdAt", "").strftime("%Y-%m-%d")
                    if instance.get("createdAt")
                    else "unknown",
            ...
            if stopped_instances:
                suggestions.append(
                    f"💡 {len(stopped_instances)} stopped instances still incurring costs - consider terminating if not needed"
                )

            if len(running_instances) > 1:
                suggestions.append(
                    "💡 Multiple running instances - consider consolidating workloads if possible"
                )

            high_cost_instances = [
                i for i in instance_details if i["bundle_details"].get("monthly_price", 0) > 10
            ]
            if high_cost_instances:
                suggestions.append(
                    f"💡 {len(high_cost_instances)} high-cost instances (>${high_cost_instances[0]['bundle_details']['monthly_price']}/month) - verify they're fully utilized"
                )
    ...
    def _get_ec2_details(self) -> Dict:
        ...
        try:
            ...
            for reservation in response.get("Reservations", []):
                for instance in reservation.get("Instances", []):
                    instance_data = {
                        ...
                        "public_ip": instance.get("PublicIpAddress", "none"),
                        "private_ip": instance.get("PrivateIpAddress", "none"),
                        "launch_time": instance.get("LaunchTime", "").strftime("%Y-%m-%d %H:%M")
                        ...
                        "tags": {
                            tag.get("Key", "unknown"): tag.get("Value", "")
            ...
            return {
                ...
                "stopped_instances": len(stopped_instances),
                "instances": instances,
                "suggestions": ["💡 Monitor instance utilization to optimize sizing"]
                if instances
                else [],
            }
        ...
        try:
            ...
            for db in response.get("DBInstances", []):
                db_data = {
                    ...
                    "created_time": db.get("InstanceCreateTime", "").strftime("%Y-%m-%d")
            ...
            return {
                ...
                "databases": databases,
                "suggestions": ["💡 Review backup retention periods and storage allocation"]
                if databases
                else [],
            }
    ...
    def _get_route53_details(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "hosted_zones": zones,
                "suggestions": ["💡 Review unused hosted zones - each costs $0.50/month"]
                if len(zones) > 1
                else [],
            }
    ...
    def _get_s3_details(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "suggestions": [
                    "💡 Review bucket contents and consider lifecycle policies for cost optimization"
                ]
                if buckets
                else [],
            }
    ...
    def get_metrics(self) -> dict:
        ...
        try:
            ...
            resources["inventory"]["lightsail"] = {
                ...
                "details": lightsail_data.get("instances", [])[:3]
                if lightsail_data.get("instances")
                else [],  # First 3 for overview
            ...
            resources["inventory"]["ec2"] = {
                ...
                "details": ec2_data.get("instances", [])[:3]
            ...
            resources["inventory"]["rds"] = {
                ...
                "details": rds_data.get("databases", [])[:3]
                if rds_data.get("databases")
                else [],  # First 3 for overview
            ...
            resources["inventory"]["s3"] = {
                ...
                "details": s3_data.get("buckets", [])[:5]
                if s3_data.get("buckets")
                else [],  # First 5 for overview
