...
METRICS_QUESTION_KEYWORDS = (
    'cost', 'aws', 'metric', 'github', 'jira', 'spend', 'usage',
    'openai', 'railway', 'performance', 'activity', 'revenue', 'expense',
)

# Stored on the assignment record — no live connector fetch required
ASSIGNMENT_METADATA_KEYWORDS = (
    'burn rate', 'monthly burn', 'team size', 'target burn', 'target monthly',
)
...
def _assignment_matches(assignment: Dict, assignment_id: str) -> bool:
    ...
    return (
        (assignment.get('id') or '').lower() == aid
        or (assignment.get('assignment_id') or '').lower() == aid
    )


def _get_enabled_connectors(metrics_config: Dict) -> List[str]:
    return [
        name for name, config in (metrics_config or {}).items()
...
def build_workspace_context(
    ...
    workspace_name = (
        workspace_data.get('name', workspace_id)
        if 'error' not in workspace_data else workspace_id
    )
...
def prepare_workspace_chat(
    ...
    enhanced_question = f"WORKSPACE CONTEXT:\n{ctx['workspace_context']}\n\nUSER QUESTION: {question}"

    metadata_answer = _assignment_metadata_answer(question, ctx)
    if metadata_answer:
        return {'short_circuit': metadata_answer, 'ctx': ctx}

    if _question_needs_live_metrics(question) and not assignment_id and not fetch_metrics and not skip_metrics_fetch:
...
def _process_rule_based(question: str, assignment_data: Dict) -> Dict[str, Any]:
    ...
    if selected_assignment and ('burn rate' in user_q_lower or 'monthly burn' in user_q_lower):
        selected_assign_obj = next(
            (a for a in assignments if a.get('name', '').lower() in selected_assignment.lower()
             or a.get('id', '').lower() in selected_assignment.lower()), None
        )
    ...
    if selected_assignment and "status" in user_q_lower:
        selected_assign_obj = next((a for a in assignments if a.get('name', '').lower() in selected_assignment.lower() or a.get('id', '').lower() in selected_assignment.lower()), None)
        if selected_assign_obj:
            assignment_name = selected_assign_obj.get('name', selected_assign_obj.get('id', 'Unknown'))
            ...
            enabled_services = [service for service, config in metrics_config.items() if config.get('enabled')]
            if enabled_services:
...
def process_question_with_workspace(
    ...
    try:
        logger.info("Processing workspace-aware question", extra={
            "user_id": user_id,
            "workspace_id": workspace_id,
            "assignment_id": assignment_id,
        ...
        prepared = prepare_workspace_chat(
            question, workspace_id, assignment_id,
            fetch_metrics=fetch_metrics,
        ...
        logger.error("Workspace-aware question processing failed", extra={
            "user_id": user_id,
            "workspace_id": workspace_id,
            "error": str(e),
        }, exc_info=True)
...
def _stream_ai_response(question: str, user_id: str, assignment_data: Dict, extra_done: Dict = None):
