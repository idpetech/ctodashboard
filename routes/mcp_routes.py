...
require_auth, require_workspace_access, optional_auth, require_web_auth, require_web_workspace_access = create_auth_decorators(user_service)
...
def get_workspace_mcp_server(workspace_id: str, user_context: dict) -> CTODashboardMCPServer:
    ...
    if server_key not in _mcp_servers:
        logger.info("Creating new MCP server instance", extra={
...
async def list_workspace_tools(workspace_id):
    ...
    try:
        ...
        logger.error("Failed to list MCP tools", extra={
            "workspace_id": workspace_id,
            "error": str(e),
            "operation": "mcp_list_tools"
        }, exc_info=e)
...
async def call_workspace_tool(workspace_id, tool_name):
    ...
    try:
        ...
        # Call the tool
        result = await mcp_server._register_handlers.__wrapped__(mcp_server).call_tool(tool_name, args)
        ...
        logger.error("Failed to call MCP tool", extra={
            "workspace_id": workspace_id,
            "tool_name": tool_name,
            "error": str(e),
            "operation": "mcp_call_tool"
...
async def list_workspace_resources(workspace_id):
    ...
    try:
        ...
        # Get resources list
        resources_result = await mcp_server._register_handlers.__wrapped__(mcp_server).list_resources()
        ...
        logger.error("Failed to list MCP resources", extra={
            "workspace_id": workspace_id,
            "error": str(e),
...
async def read_workspace_resource(workspace_id, resource_uri):
    ...
    try:
        ...
        # Read the resource
        result = await mcp_server._register_handlers.__wrapped__(mcp_server).read_resource(resource_uri)
        ...
        logger.error("Failed to read MCP resource", extra={
            "workspace_id": workspace_id,
...
async def workspace_chatbot(workspace_id):
    ...
    try:
        ...
        logger.error("Workspace chatbot failed", extra={
            "workspace_id": workspace_id,
            "error": str(e),
            "operation": "workspace_chatbot"
        }, exc_info=e)
...
def forbidden(error):
    return jsonify({
        "error": "Forbidden", 
        "message": "Insufficient permissions for workspace access"
    }), 403
